...
# Load VMs definitions and provisioning global configuration
configuration = JSON.load(File.new(configuration_file))
machines = configuration.fetch('machines', {})
provisioning = configuration.fetch('provisioning', {})
    ...
    config.vm.define node_name do |server|
      ...
      node['networks'].each do |net_name, network|
        server.vm.network network.fetch('type', :private_network),
                          ip: network['ip'],
                          netmask: network.fetch('netmask', '255.255.255.0'),
                          mac: node['mac'],
                          auto_config: network.fetch('auto_config', true)
